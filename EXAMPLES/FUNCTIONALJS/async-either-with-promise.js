...
Promise.all([suitablePersonEnterGayBar(), unsuitablePersonEnterGayBar()])
  .then(result =>
    result.map(
      (person, i) =>
        `Person ${i + 1}: ${person.cost || ''}${person.reasons.join(' ')}`,
    ),
  )
...
function costToEnter(person, rules) {
  ...
  return Promise.all(rules.map(fn => fn(person).catch(err => err))).then(
    results => {
...
function checkClothes(person) {
  if (person.gender === 'Male' && person.clothes.indexOf('Tie') !== -1)
    return Promise.reject(new Error('Smarten up!'));
  if (person.gender === 'Female' && person.clothes.indexOf('Trainers') !== -1)
    return Promise.reject(new Error('Smarten up!'));
